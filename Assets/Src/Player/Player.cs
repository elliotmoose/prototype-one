...
public class Player : Entity
{
    ...
    private void FixedUpdateMovement()
    {
        ...
        if (isMoving)
        {
            ...
            var sprintSpeed = _isSprinting ? sprintMultiplier : 1;

            if (_isSprinting)
            {
                GetComponent<PlayerEnergy>().UseEnergy(50 * Time.fixedDeltaTime);
            }

            GetComponent<CharacterController>().Move(delta3 * this.movementSpeed * sprintSpeed * Time.fixedDeltaTime);
        }
    ...
    override public void Die(TakeDamageInfo damageInfo)
    {
        Time.timeScale = 0;
