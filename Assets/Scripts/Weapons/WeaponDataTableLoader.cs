...
public class WeaponDataTableLoader : MonoBehaviour
{
  ...
  private static bool verboseLoad = false;
  void Awake()
  {
    if (loaded)
    {
      ...
      for (int i = 1; i < rows.Length; i++)
      {
        ...
        for (int j = 0; j < cells.Length; j++)
        {
          ...
          if (cell != "")
          {
            if (j == 0)
            {
              switch (cell)
              {
                ...
                case "LASER":
                  ...
                  if (verboseLoad) Debug.Log("Found Weapon:" + cell);
                  break;
                //found property key                        
                default:
                  if (verboseLoad) Debug.Log($"Found property {cell}");
            ...
            {
              if (verboseLoad) Debug.Log($"Found value {cell} for property {currentWeaponProperty.name}");
    ...
    {
      if (verboseLoad) Debug.LogWarning("Table has no rows");
    }
